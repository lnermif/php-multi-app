...
class MultiApp
{
    ...
    protected function parseMultiApp(): bool
    {
        ...
        if ($appName || ($scriptName && !in_array($scriptName, ['index', 'router', 'think']))) {
            ...
            if (!$this->app->http->isBind()) {
                ...
                if (isset($map[$name])) {
                    ...
                    if($nameArr){
                        $appPath = $this->app->http->getPath() ?: $this->app->getBasePath() . $nameArr[1]. DIRECTORY_SEPARATOR . $appName . DIRECTORY_SEPARATOR;
                    }else {
                ...
                if ($name) {
                    if($nameArr){
                        $this->app->request->setRoot('/' . $nameArr[1] .'/'. $nameArr[0]);
                    }else {
        ...
        $this->setApp($appName ?: $defaultApp, $nameArr);
        return true;
    }
     ...
     */
    protected function setApp(string $appName, array $nameArr = []): void
    {
        ...
        // 设置应用命名空间
        if($nameArr){
            $this->app->setNamespace($this->app->config->get('app.app_namespace') ?: 'app\\' .$nameArr[1].'\\'. $appName);
        }else {
